...
import { 
  ...
  Tag, Filter, Grid, List, ArrowRightLeft, ShieldAlert, Zap, Info
} from 'lucide-react';
...
export const AffinityMatrixModal: React.FC<AffinityMatrixModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white dark:bg-[#121216] rounded-3xl shadow-2xl border border-zinc-200 dark:border-zinc-800 max-w-5xl w-full overflow-hidden flex flex-col max-h-[92vh]">
        
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-[#16161c]">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-bold text-zinc-900 dark:text-zinc-100 text-base font-display">
                  Matriz Avançada de Afinidades & Desafinidades
                </h3>
              ...
              <p className="text-xs text-zinc-600 dark:text-zinc-400">
                Configure pesos de proximidade (Alta, Média, Baixa) e distanciamento (Crítica, Moderada, Leve) por categoria.
              </p>
            </div>
          </div>
        ...
        {isAddingCat && (
          <div className="px-6 py-3 bg-emerald-50/80 dark:bg-[#181822] border-b border-emerald-200 dark:border-emerald-900/50 flex items-center justify-between animate-in slide-in-from-top-2">
            <form onSubmit={handleCreateCategory} className="flex flex-wrap items-center gap-3 w-full">
              ...
              >
                <option value="affinity">Para Afinidade (Aproximar)</option>
                <option value="antiAffinity">Para Desafinidade (Afastar)</option>
              </select>
              <button
                type="submit"
        ...
        {viewMode === 'focused' ? (
          <div className="grid grid-cols-1 md:grid-cols-12 overflow-hidden flex-1">
            ...
            <div className="md:col-span-4 border-r border-zinc-200 dark:border-zinc-800 p-4 flex flex-col bg-zinc-50 dark:bg-[#141419]/80">
              <div className="relative mb-3">
                ...
                <input
                  ...
                  className="w-full pl-8 pr-3 py-2 bg-white dark:bg-[#18181f] border border-zinc-300 dark:border-zinc-700/60 rounded-xl text-xs text-zinc-900 dark:text-zinc-200 placeholder-zinc-400 dark:placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                />
              </div>

              <div className="overflow-y-auto space-y-1.5 flex-1 pr-1 max-h-[55vh] md:max-h-full">
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left p-2.5 rounded-2xl text-xs font-medium transition-all flex items-center justify-between cursor-pointer border ${
                        isSelected
                          ? 'bg-emerald-100 dark:bg-emerald-950/70 border-emerald-500 dark:border-emerald-600/80 text-emerald-950 dark:text-emerald-200 shadow-xs'
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1 shrink-0 ml-1">
                        {affList.length > 0 && (
                          <span className="text-[10px] px-1.5 py-0.5 rounded-full font-bold bg-emerald-100 dark:bg-emerald-900/60 text-emerald-800 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-700/50">
                            +{affList.length}
                          </span>
                        )}
                        {antiList.length > 0 && (
                          <span className="text-[10px] px-1.5 py-0.5 rounded-full font-bold bg-rose-100 dark:bg-rose-900/60 text-rose-800 dark:text-rose-300 border border-rose-300 dark:border-rose-700/50">
                            -{antiList.length}
                          </span>
                        )}
                      </div>
            ...
            <div className="md:col-span-8 p-5 overflow-y-auto flex flex-col bg-white dark:bg-[#121216]">
              {currentStudent ? (
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between bg-zinc-50 dark:bg-[#181820] p-4 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-xs">
                    ...
                    <div className="flex items-center gap-2 text-xs">
                      <span className="px-3 py-1 rounded-xl bg-emerald-100 dark:bg-emerald-950/70 text-emerald-800 dark:text-emerald-300 font-bold border border-emerald-300 dark:border-emerald-800/60 flex items-center gap-1.5">
                        <Heart className="w-3.5 h-3.5 fill-emerald-600 dark:fill-emerald-400" />
                        {currentAffDetails.length} afinidades
                      </span>
                      <span className="px-3 py-1 rounded-xl bg-rose-100 dark:bg-rose-950/70 text-rose-800 dark:text-rose-300 font-bold border border-rose-300 dark:border-rose-800/60 flex items-center gap-1.5">
                        <AlertTriangle className="w-3.5 h-3.5 fill-rose-600 dark:fill-rose-400" />
                        {currentAntiDetails.length} desafinidades
                      </span>
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <h5 className="text-xs font-bold uppercase tracking-wider text-zinc-700 dark:text-zinc-400">
                        Configurar Relação com Colegas da Turma:
                      </h5>
                          ...
                          >
                            <div className="flex items-center justify-between gap-3">
                              ...
                              <div className="flex items-center gap-1.5 shrink-0">
                                ...
                                >
                                  <Heart className={`w-3.5 h-3.5 ${isAff ? 'fill-white text-white' : 'text-emerald-600 dark:text-emerald-400'}`} />
                                  <span>{isAff ? 'Afinidade' : '+ Afinidade'}</span>
                                </button>
                                ...
                                >
                                  <AlertTriangle className={`w-3.5 h-3.5 ${isAnti ? 'fill-white text-white' : 'text-rose-600 dark:text-rose-400'}`} />
                                  <span>{isAnti ? 'Desafinidade' : '- Desafinidade'}</span>
                                </button>
                              </div>
                            </div>

                            {/* Detailed Sub-Configuration when Affinity is Active */}
                            {isAff && (
                            ...
                            {/* Detailed Sub-Configuration when Anti-Affinity is Active */}
                            {isAnti && (
          ...
          /* Matrix Full Grid View */
          <div className="p-5 overflow-auto flex-1 bg-white dark:bg-[#121216]">
            <div className="mb-3 flex items-center justify-between text-xs">
              <p className="text-zinc-600 dark:text-zinc-400 font-medium">
                Clique nas células para alternar entre: <span className="text-zinc-500 font-bold">Neutro</span> → <span className="text-emerald-700 dark:text-emerald-400 font-bold">Afinidade (+3)</span> → <span className="text-rose-700 dark:text-rose-400 font-bold">Desafinidade (-3)</span>
              </p>
              <div className="flex items-center gap-3 font-semibold">
                <span className="flex items-center gap-1 text-[11px] text-emerald-700 dark:text-emerald-400">
                  <span className="w-2.5 h-2.5 rounded-full bg-emerald-500"></span> Afinidade
                </span>
                <span className="flex items-center gap-1 text-[11px] text-rose-700 dark:text-rose-400">
                  <span className="w-2.5 h-2.5 rounded-full bg-rose-500"></span> Desafinidade
                </span>
              </div>
            </div>

            <div className="overflow-x-auto border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-xs">
                          ...
                          return (
                            <td key={colStudent.id} className="p-1 border-b border-r border-zinc-200 dark:border-zinc-800 text-center">
                              <button
                                ...
                                title={`${rowStudent.name} → ${colStudent.name}: ${
                                  aff ? `Afinidade (${aff.level} / ${aff.category})` : anti ? `Desafinidade (${anti.level} / ${anti.category})` : 'Neutro'
                                }`}
                              >
                                {aff ? '+3' : anti ? '-3' : '•'}
                              </button>
        ...
        {/* Footer */}
        <div className="flex items-center justify-between px-6 py-3.5 border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-[#16161c]">
          <div className="flex items-center gap-4 text-xs text-zinc-600 dark:text-zinc-400 font-semibold">
            ...
            <span className="hidden sm:inline text-zinc-500">
              Ponderado no algoritmo de distribuição
            </span>
          </div>
          ...
          <button
            type="button"
            onClick={onClose}
            className="px-6 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-xl shadow-xs shadow-emerald-950/40 cursor-pointer transition-colors"
          >
            Concluir Edição
          </button>
        </div>
