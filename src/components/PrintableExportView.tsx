    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      ...
      <div className="bg-white dark:bg-[#121216] p-4 sm:p-6 rounded-3xl border border-slate-200 dark:border-zinc-800 shadow-xs flex flex-col sm:flex-row items-center justify-between gap-4 no-print transition-colors">
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          ...
          {/* Botão para Incluir/Ocultar Fotos dos Alunos no Espelho */}
          <button
            type="button"
            id="toggle-student-photos-btn"
            onClick={() => setShowPhotos(prev => !prev)}
            className={`flex items-center gap-2 px-3.5 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer border shadow-xs ${
              showPhotos 
                ? 'bg-purple-50 hover:bg-purple-100/80 dark:bg-purple-950/40 dark:hover:bg-purple-950/60 text-purple-900 dark:text-purple-300 border-purple-300 dark:border-purple-700/60 ring-2 ring-purple-500/20' 
                : 'bg-slate-100 hover:bg-slate-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-slate-600 dark:text-zinc-400 border-slate-300 dark:border-zinc-700/60'
            }`}
            title={showPhotos ? "Clique para ocultar as fotos dos alunos no espelho e no PDF" : "Clique para incluir as fotos dos alunos no espelho e no PDF"}
            aria-pressed={showPhotos}
          >
            <Camera className={`w-4 h-4 ${showPhotos ? 'text-purple-600 dark:text-purple-400' : 'text-slate-400'}`} />
            <span>Fotos no Espelho:</span>
            <span className={`px-2 py-0.5 rounded-md text-[11px] font-black uppercase tracking-wider ${
              showPhotos 
                ? 'bg-purple-200/90 dark:bg-purple-900/80 text-purple-950 dark:text-purple-100' 
                : 'bg-slate-200 dark:bg-zinc-700 text-slate-500 dark:text-zinc-400 line-through'
            }`}>
              {showPhotos ? 'Ativadas' : 'Desativadas'}
            </span>
          </button>
          ...
          >
            {Array.from({ length: rows }).map((_, rIdx) =>
              Array.from({ length: cols }).map((_, cIdx) => {
                ...
                const cardHeightClass = showPhotos
                  ? isUltraCompact 
                    ? 'min-h-[66px] h-auto p-1' 
                    : isCompact 
                    ? 'min-h-[84px] h-auto p-1' 
                    : isMediumCompact 
                    ? 'min-h-[106px] h-auto p-1.5' 
                    : 'min-h-[136px] h-auto p-2 sm:p-2.5'
                  : isUltraCompact 
                    ? 'h-10 p-0.5' 
                    : isCompact 
                    ? 'h-14 p-1' 
                ...
                if (!isActive) {
                  return (
                    <div
                      key={deskId}
                      className={`${cardHeightClass} invisible pointer-events-none`}
                      aria-hidden="true"
                    />
                  );
                }
                  ...
                  return (
                    ...
                    >
                      {showPhotos && (
                        <div className={`${
                          isUltraCompact 
                            ? 'w-6 h-8 rounded-sm mb-1' 
                            : isCompact 
                            ? 'w-8.5 h-11.5 rounded-md mb-1' 
                            : isMediumCompact 
                ...
                // ALUNOS: Cartões brancos com cantos arredondados, sombras sutis, foto retangular vertical (3:4) e nome centralizados
                return (
                  <div
                    key={deskId}
                    className={`${cardHeightClass} rounded-lg sm:rounded-xl bg-white border border-slate-200 shadow-xs flex flex-col items-center justify-between text-center transition-all hover:border-slate-300`}
                  >
                    {/* Top: Número da chamada centralizado (condicional) */}
                    {showRollNumbers ? (
                      <span className={`${
                        isUltraCompact 
                          ? 'text-[6.5px] px-1 py-0 mb-0.5' 
                          : isCompact 
                          ? 'text-[7.5px] px-1.5 py-0.2 mb-0.5' 
                          : 'text-[9.5px] px-2 py-0.5 mb-1'
                      } font-bold text-slate-600 bg-slate-100 rounded-full inline-flex items-center justify-center leading-none shrink-0`}>
                    ...
                    {/* Center: Foto do Aluno em formato retangular vertical (3:4) com cantos arredondados */}
                    {showPhotos && (
                      <div 
                        className={`${
                          isUltraCompact 
                            ? 'w-6 h-8 rounded-sm my-0.5' 
                            : isCompact 
                            ? 'w-8.5 h-11.5 rounded-md my-0.5' 
                            : isMediumCompact 
                            ? 'w-11 h-15 rounded-lg my-1' 
                      ...
                      >
                        {(student.photoUrl || student.foto || (student as any).avatar || (student as any).image) ? (
                          ...
                          >
                            <span className={`${isUltraCompact ? 'text-[8px]' : isCompact ? 'text-[10px]' : isMediumCompact ? 'text-xs' : 'text-base'}`}>
                    ...
                    {/* Bottom: Nome do aluno centralizado - Fonte reduzida quando fotos estão ativadas e fixado na base */}
                    <div className="w-full px-0.5 overflow-hidden flex flex-col items-center justify-center mt-auto pb-0.5 sm:pb-1">
                      <p 
                        className={`${
                          showPhotos
                            ? (isUltraCompact
                                ? 'text-[6px] sm:text-[6.5px] leading-[1.08]'
                                : isCompact
                                ? 'text-[7px] sm:text-[7.5px] leading-tight'
                                : isMediumCompact
                                ? 'text-[8.5px] sm:text-[9px] leading-tight'
                                : 'text-[9.5px] sm:text-[10.5px] leading-tight')
                            : (isUltraCompact
                      ...
                      {student.nickname && !isCompact && !isUltraCompact && !showPhotos && (
